...
const Navbar = ({ toggleSidebar }) => {
  ...
  useEffect(() => {
    ...
    // Sync user if modified on Profile page
    window.addEventListener('storage', loadUser);
    window.addEventListener('ems-user-changed', loadUser);
    return () => {
      window.removeEventListener('storage', loadUser);
      window.removeEventListener('ems-user-changed', loadUser);
    };
  }, []);
    ...
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white/80 px-4 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/80 sm:px-6 lg:px-8">
      <div className="flex items-center">
        <button
          ...
          className="mr-4 rounded-md p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200 lg:hidden"
        >
          <span className="sr-only">Open sidebar</span>
          <Menu className="h-6 w-6" aria-hidden="true" />
        </button>
        <div className="hidden lg:block lg:text-xl lg:font-bold lg:text-slate-800 lg:dark:text-white">
          Dashboard
        </div>
      </div>
